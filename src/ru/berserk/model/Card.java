...
class Card {
    ...
    public static Card getCardByName(String name) {
        ...
        switch (name) {
            ...
            case "Свирепый резак":
                return new Card(0, name, "", 2, 0, 0, 1, "ТАПТ:2 Выбранное существо получает 'Опыт в атаке и Рывок'.", 0, 28);
            case "Эндор Флем":
            ...
            case "Огненный щит":
                return new Card(2, name, "", 2, 1, 1, 0, "Выбранное существо до конца следующего хода получает 'Не получает от ударов ран'. Взять карт 1.", 0, 0);
            case "Орк-егерь":
    ...
    static void ability(Gamer owner, Card _who, Player _whis, Creature _cr, Player _pl, String txt) throws IOException {
        ...
        if (txt.contains("Посмотрите топдек противника, можете положить его на кладбище")) {
            ...
            owner.sendChoiceYesNo(_who.name + " предлагает сбросить топдек противника.", _whis.owner.opponent.player.deck.topDeck().name, "Сбросить", "Оставить");
            ...
            if (owner.choiceYesNo == 1) {
                ...
                owner.printToView(0, _who.name + " сбрасывает верхнюю карту с колоды " + _whis.owner.opponent.player.playerName);
                owner.opponent.printToView(0, _who.name + " сбрасывает верхнюю карту с колоды " + _whis.owner.opponent.player.playerName);
            }
        }
        ...
        {//Only here - _cr=_who to get access to creature
            _whis.effect.takeBBShield(true);
        }
        if (txt.contains("Лики-абилка."))
        ...
        {//Поиск (type,color,creatureType,cost,costEx,name)
            if (_whis.playerName.equals(owner.name)) {
                ...
                if (owner.choiceXname.equals(" ")) owner.choiceXname = "";
