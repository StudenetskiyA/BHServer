...
class Card {
	...
	void playCard(Player _who, Permanent _target) throws IOException{
		ability(this, _who, _target, this.text);	
	}
	...
	public static Card createCardByName(String name) {
       ...
       //New       
	    ...
	    case "Дух эльфа":
            return new Card(3, name, "", 6, 2, 3, 0, "Магический урон. Гибельт: Нанести магией ран 2.", 3, 2);
	    case "Ходячий камень":
	...
	static void ability(Card _whatCard, Permanent _who, Permanent _target, String txt) throws IOException {
		Gamer owner = _who.ownerGamer;
		...
		if (txt.contains("Физический удар-убийство на ") ) {
			...
			//if (_cr != null) {
				_target.takeDamage(dmg, _who, Creature.DamageSource.physic);
				if (_target.isDie()) {
					_who.killing();					
                    ...
                    owner.sendChoiceForSpell(3, 0, _whatCard.name + " просит выбрать цель.");
					...
					ability(_whatCard, _who, owner.choicePermanent, "Нанести физикой ран "+dmg);
				}
		}
		...
		if (txt.contains("Нанести магией ран ")) {
			int dmg = MyFunction.getNumericAfterText(txt, "Нанести магией ран ");
				_target.takeDamage(dmg, _who, Creature.DamageSource.magic);
				if (_target.isDie()) _who.killing();
		}
...
//				_pl.takeDamage(dmg,_who,DamageSource.magic);
//			}
//		}
...
//			_whis.owner.opponent.player.takeDamage(dmg,_who,DamageSource.magic);
//
//		}
...
//			_cr.takeDamage(dmg, _who, Creature.DamageSource.ability);
//			_whis.takeDamage(dmg,_who,DamageSource.magic);
//		}
