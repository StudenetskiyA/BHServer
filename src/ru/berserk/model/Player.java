...
public class Player extends Card {
	...
	int getNumberOfCreature(Creature _cr) {
		for (int i = 0; i < creatures.size(); i++) {
			if (creatures.get(i).id.equals(_cr.id))
				return i;
		}
		return -1;
	}
	...
	void endTurn() throws IOException {
		...
		// if (owner.opponent.player.effect.getBBShield())
		// owner.opponent.player.effect.takeBBShield(false);

		if (untappedCoin > totalCoin)
	...
	ArrayList<Creature> searchWhenOtherDieAbility(Creature cr) {
		...
		for (Creature p : creatures) {
			if (p.text.contains("При гибели другого вашего существа:") && p != cr && !p.isDie())
				tmp.add(p);
			if (p.text.contains("При гибели в ваш ход другого вашего существа:")
					&& p.owner.playerName.equals(owner.board.whichTurn) && p != cr && !p.isDie())
				tmp.add(p);
		}
		return tmp;
	}
		...
		while (temp.hasNext()) {
			...
			if (tmp.text.contains("Гибельт:") && !tmp.effects.deathPlayed) {
				...
				if (MyFunction.canTargetComplex(this, tmp)) {
					...
					tmp.effects.deathPlayed = true;// If you can't target, after
													// you can't play this
													// ability
				}
			}
	...
	void massUpkeepCheckNeededTarget() throws IOException {// if someone wants
															// to choice target
															// at death(self or
															// other) - pause
															// game
		...
		while (temp.hasNext()) {
			...
			if (tmp.text.contains("В начале хода")
					|| tmp.text.contains("В начале вашего хода") && tmp.getTougness() > tmp.damage)
	...
	void massSummonCheckNeededTarget() throws IOException {// if someone wants
															// to choice target
															// at death(self or
															// other) - pause
															// game
		...
		while (temp.hasNext()) {
			...
			if (tmp.text.contains("Наймт:") && !tmp.effects.battlecryPlayed && !tmp.isDie())
				...
				if (MyFunction.canTargetComplex(this, tmp, MyFunction.getTextBetweenSymbol(tmp.text, "Наймт:", "."))) {
					...
					if (tmp.text.contains("Цель не обязательно."))
						owner.activatedAbility.ableAbility = false;
					else
					...
					owner.printToView(0, "Целей для " + tmp.name + " нет.");
					tmp.effects.battlecryPlayed = true;// If you can't target,
														// after you can't play
														// this ability
				}
		}
	}
	...
	void newTurn() throws IOException {
		...
		// this.equpiment[3].name.equals("Пустошь Тул-Багара")) {
		...
		// owner.opponent.player.equpiment[3].name.equals("Пустошь Тул-Багара"))
		// {
		// owner.printToView(0, "Пустошь Тул-Багара ранит всех героев.");
		// this.takeDamage(1,DamageSource.magic);
		// owner.opponent.player.takeDamage(1,DamageSource.magic);
		...
		// Draw
		if (owner.board.turnCount != 1) {// First player not draw card in first
											// turn. It's rule.
			if (deck.haveTopDeck())
				drawCard();
			else {
				...
				owner.printToView(0,
						playerName + " не смог взять карту в начале хода(" + cantDrawCardAtBeginTurn + ")");
	...
	Card getCardByID(String id) {
		for (int i = 0; i < cardInHand.size(); i++) {
			if (cardInHand.get(i).id.equals(id))
	...
	void playCard(int num, Card _card, Creature _targetCreature, Player _targetPlayer) throws IOException {
		...
		if (untappedCoin >= effectiveCost) {
			...
			if (_card.type == 1) {
				...
				// #PlaySpell(Player, SpellName,
				// TargetHalfBoard[0-self,1-enemy], TargetCreatureNum[-1 means
				// targets player])
				...
				if (_targetCreature != null) {
					if (_targetCreature.owner == this)
						owner.sendBoth("#PlaySpell(" + playerName + "," + _card.name + ",0,"
								+ getNumberOfCreature(_targetCreature) + ")");
					else
